...
extern "C"
{
    ...
    producer_event_t producerTick(const uint32_t ts)
    {
        ...
        return evt; // this function is disabled, do not use
    ...
    {
        producer_event_t evt = {0};
        evt.ready = false;

        /* Check for producer callbacks */
        if (!g_node ||
            !g_node->getSubModule ||
            !g_node->getRuntime)
        { /* return error if callback lookup fails */
            evt.error = true;
            return evt;
        }

        subModule_t *sub = g_node->getSubModule(subIdx);
        if (!sub)
        {
            evt.error = true;
            printf("[PRODUCER] Invalid sub-module index: %u\n", subIdx);
            return evt;
        }
        ...
        if (!(sub->producer_flags & PRODUCER_FLAG_ACTIVE))
        {
            printf("[PRODUCER] Producer disabled for sub-module %u flags 0x%02X\n", subIdx, sub->producer_flags);
            return evt;
        }
        /* Publishing disabled? */
        if (!(sub->producer_flags & PRODUCER_FLAG_PUBLISH_ENABLED))
        {
            evt.ready = false;
            printf("[PRODUCER] Publishing disabled for sub-module %u flags 0x%02X\n", subIdx, sub->producer_flags);
            return evt;
        }

        /* Rate limiting */
        // if (ts - lastProducerTick[subIdx] < sub->producer_period_ms)
        //     return evt;
        ...
        if ((sub->producer_flags & PRODUCER_FLAG_CHANGE_ONLY) &&
            (value == rt->last_published_value))
        {
            printf("[PRODUCER] Sub-module %u not changed\n", subIdx);
            evt.ready = false;
            return evt;
        }
